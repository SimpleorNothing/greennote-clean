...
6. 결과 화면에서 **「판서 저장하고 홈으로」** 버튼을 누르면 올린 판서 이미지를 **R2에 저장**하고(과목·단원·요약·키워드를 `index.json`에 함께 기록) **홈으로 이동**한다.
7. **홈 화면(상단 업로드 영역 아래)에는 저장된 판서가 그리드로 쭉 표시**된다. 썸네일을 누르면 원본을 크게 볼 수 있다.
8. 결과 화면에서 **같은 범위로 새 5문제**로 더 풀거나, **저장 안 하고 홈으로** 돌아갈 수도 있다.
...
- Worker(`src/index.js`)가 API를 처리한다:
  ...
  - `GET/PUT/DELETE /api/image/{id}` — 판서 사진 (결과 화면의 「판서 저장하기」로 저장)
...
- 사진: 업로드 시 1200px·JPEG로 압축해 전송한다. 문제 생성 단계에서는 저장하지 않고, 결과 화면에서 **「판서 저장하기」를 누를 때** R2(`img/{id}`)에 저장한다.
