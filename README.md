...
2. Gemini Vision이 판서를 읽어 **과목·단원·핵심 요약**을 파악하고, 올린 판서 이미지를 **출제한 문제와 함께 자동으로 R2에 저장**한다(과목·단원·요약·키워드·문제를 `index.json`에 함께 기록).
...
6. 결과 화면에서 **같은 범위로 새 5문제**로 더 풀거나, **홈으로** 돌아간다.
7. **홈 화면(상단 업로드 영역 아래)에는 저장된 판서가 그리드로 쭉 표시**된다.
   - 썸네일을 **짧게 누르면** 원본과 함께 **「📋 이 판서 문제 다시 풀기」**(저장해 둔 그 문제) · **「✨ 유사한 추가 문제」**(같은 범위의 새 문제) 버튼이 뜬다.
   - **길게 누르면(롱프레스) 우측 상단에 × 삭제 버튼**이 나타난다(데스크톱은 마우스를 올리면 표시). ×를 누르면 해당 판서가 삭제된다.
...
- Worker(`src/index.js`)가 API를 처리한다:
  ...
  - `GET/PUT/DELETE /api/image/{id}` — 판서 사진 (업로드 시 자동 저장)
...
- 사진: 업로드 시 1200px·JPEG로 압축해 전송하고, 문제를 출제하면서 **자동으로** R2(`img/{id}`)에 저장한다.
