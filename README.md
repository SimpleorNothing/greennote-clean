...
2. Gemini Vision이 판서를 읽어 **과목·단원·핵심 요약**을 파악하고, 올린 판서 이미지를 **자동으로 R2에 저장**한다(과목·단원·요약·키워드를 `index.json`에 함께 기록).
...
7. **홈 화면(상단 업로드 영역 아래)에는 저장된 판서가 그리드로 쭉 표시**된다. 썸네일을 짧게 누르면 원본을 크게 보고, **길게 누르면(롱프레스) 우측 상단에 × 삭제 버튼**이 나타난다(데스크톱은 마우스를 올리면 표시). ×를 누르면 해당 판서가 삭제된다.
